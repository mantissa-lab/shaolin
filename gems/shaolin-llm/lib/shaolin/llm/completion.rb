module Shaolin
  module LLM
    ...
    class Completion
      attr_reader :text, :reasoning, :tool_calls, :usage, :data

      def initialize(text: nil, reasoning: nil, tool_calls: [], usage: {}, data: nil)
        @text = text
        @reasoning = reasoning
      ...
      def data? = !@data.nil?
      def to_h = { text: text, reasoning: reasoning, tool_calls: tool_calls, usage: usage, data: data }
    end
  end
end
